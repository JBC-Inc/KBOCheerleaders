...
source(system.file("R", "cheer_data.R", package = "KBOCheerleaders"))
...
ui <- bslib::page_sidebar(
  ...
  title = shiny::titlePanel(
    shiny::tags$div(
      shiny::column(
        width = 12,
        shiny::tags$img(src = "https://upload.wikimedia.org/wikipedia/en/thumb/5/59/KBO_League.svg/1920px-KBO_League.svg.png",
                        height = "50px;",
                        style = "float: left;"),
        shiny::tags$h1("Cheerleaders!", style = "float: right; margin-left: 10px;")
        )
      ),
    windowTitle = "KBO"
    ),
...
server <- function(input, output, session) {
  ...
  # Render the Team Roster photograph

  output$teamPhoto <- shiny::renderUI({
    page <- team_page()
    photo_url <- getTeamPhoto(page)
    team <- names(teams)[teams == input$team]
    if (team != "Samsung Lions") {
      shiny::img(src = photo_url, height = "100%")
    } else {
      shiny::img(src = "https://www.samsunglions.com/en/img/img_cheerleader2017_en.jpg", height = "100%")
    }
  })

  # Render the cheerleader photograph
  ...
  # Render the cheerleader biography

  output$cheerleaderBio <- DT::renderDataTable(expr = {

    bio_table <- cheerleader_page()$bio_table
    df_tables <- cheerleader_page()$df_tables

    bio_data <- parseCheerleaderBio(bio_table, df_tables)

    bio_data <- bio_data |>
      dplyr::mutate(X1 = stringr::str_to_title(X1)) |>
      dplyr::mutate(X2 = ifelse(
        !stringr::str_detect(X1, "Link|Site"),
...
parseCheerleaderBio <- function(bio_table, df_tables) {
  ...
  if (ncol(table) == 2) {
    table |>
      dplyr::filter((X1 != X2)) |>
      dplyr::filter(!X1 %in% c("support team", "SNS", "platform", "signature"))
